...
window.AlgoWidgets[4] = function (container) {
  ...
  function generateSteps() {
    var points = [];
    var numPoints = 8 + Math.floor(Math.random() * 3); 
    for (var i = 0; i < numPoints; i++) {
      points.push({
        id: i,
        x: 20 + Math.floor(Math.random() * 460), 
        y: 20 + Math.floor(Math.random() * 260)  
      });
    }
    ...
    function render() {
      ...
      Object.keys(graph).forEach(node => {
        ...
        if (s.distances[node] !== Infinity) {
          ...
          distText.setAttribute('y', nodePositions[node].y + 35); 
      ...
      s.queue.forEach(node => {
        ...
        queueNode.className = 'algo-node-in-queue'; 
