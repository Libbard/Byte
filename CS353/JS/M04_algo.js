...
window.AlgoWidgets[3] = function (container) {
  ...
  function render() {
    ...
    for (var i = 0; i < s.edges.length; i++) {
      ...
      var d = 'M ' + u.x + '% ' + (u.y * 2.5 - 20 + 20) + ' L ' + v.x + '% ' + (v.y * 2.5 - 20 + 20); 
      var dash = '';
      var marker = 'url(#arrow3)';

      if (edge.type === 'tree') { stroke = 'var(--algo-sorted)'; }
      if (edge.type === 'back') { stroke = 'var(--brand-500)'; dash = 'stroke-dasharray="4,4"'; marker = 'url(#arrow3b)'; d = 'M ' + u.x + '% ' + (u.y * 2.5) + ' Q 10% 125 ' + v.x + '% ' + (v.y * 2.5 - 20 + 20); } 
      if (edge.type === 'forward') { stroke = 'var(--algo-swap)'; dash = 'stroke-dasharray="6,2"'; marker = 'url(#arrow3c)'; d = 'M ' + u.x + '% ' + (u.y * 2.5 - 20 + 20) + ' Q 60% 125 ' + v.x + '% ' + (v.y * 2.5 - 20 + 20); } 
