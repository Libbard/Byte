...
(function () {
  ...
  const SM2_REVIEW_INTERVALS = [3, 7]; 
  ...
  function buildSkeleton(curriculumMap, rawUserConfig) {
    ...
    const mps           = Math.max(0.5, userConfig.modules_per_session); 
    ...
    for (const [courseId, examDate] of Object.entries(courseExamMap)) {
      const goldenDate = findGoldenReviewDate(userConfig, examDate); 
  ...
  function materializePlan(curriculumMap, rawUserConfig, skeleton, repairedResult, lang = 'ar') {
    ...
    for (const d of skeleton.days) {
      byDate[d.date] = {
        ...
        day_type   : d.day_type,
        week_number: Math.floor(daysBetween(firstDate, d.date) / 7) + 1,  
        sessions   : []
      };
    }
