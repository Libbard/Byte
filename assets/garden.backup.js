...
;(function() {
  ...
  function buildQueue() {
    ...
    fc.pos = 0;
    fc.totalOriginal = fc.queue.length; 
    fc.completed = 0;
  ...
  const GARDEN_AI_ENDPOINT = 'https://garden-ai.xxli50xx.workers.dev'; 
  ...
  function extractCardContent(card) {
    ...
    const activeLayer = card.querySelector('.depth-layer.active');
    if (activeLayer) {
      const tpl = activeLayer.querySelector(`.content-${L}`);
      result.content = (tpl?.innerHTML || activeLayer.textContent || '').replace(/<[^>]+>/g, '').trim();
    } else {
      
      const texts = [];
      card.querySelectorAll('[data-bilingual]').forEach(el => {
        const tpl = el.querySelector(`.content-${L}`);
        if (tpl) texts.push(tpl.innerHTML.replace(/<[^>]+>/g, '').trim());
      });
    ...
    if (svg) {
      const prompt = svg.getAttribute('data-diagram-prompt') || '';
      if (prompt) result.content += '\n\n[SVG Diagram Description]: ' + prompt;
    }

    
    const algoPlaceholder = card.querySelector('.svg-placeholder');
    if (algoPlaceholder) {
      const prompt = algoPlaceholder.getAttribute('data-diagram-prompt') || '';
      if (prompt) result.content += '\n\n[Algorithm Visualization]: ' + prompt;
    }
  ...
  function buildPrompt(cardData) {
    ...
    const systemPrompt = currentLang === 'ar'
      ? `أنت أستاذ جامعي ودود متخصص في علوم الحاسب. اشرح المفهوم التالي بطريقة بسيطة وواضحة مع أمثلة عملية. استخدم العربية الفصحى البسيطة. كن مختصراً ومفيداً — لا تتجاوز 300 كلمة. المادة: ${subject}, الوحدة: ${module}.`
      : `You are a friendly CS professor. Explain the following concept simply and clearly with practical examples. Be concise and helpful — max 300 words. Course: ${subject}, Module: ${module}.`;

    const userMsg = currentLang === 'ar'
      ? `اشرح لي هذا المفهوم:\n\nالعنوان: ${cardData.title}\n\nالمحتوى:\n${cardData.content.substring(0, 1500)}`
    ...
    overlay.innerHTML = `
      <div class="ai-modal">
        <div class="ai-modal-header">
          <h3>✨ ${aiT('شرح بالذكاء الاصطناعي', 'AI Explanation')}</h3>
  ...
  function initAiExplain() {
    const targets = document.querySelectorAll(
      '.concept-card, .professor-card, .vault-section, .objectives-card'
    );
    targets.forEach(card => {
      ...
      btn.innerHTML = '✨';
      btn.title = aiT('شرح بالذكاء الاصطناعي', 'AI Explanation');
